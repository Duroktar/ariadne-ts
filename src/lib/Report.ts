...
export class Report<S extends Span> implements iReport<S> {
  ...
  get_source_groups(cache: Cache<S['SourceId']>): SourceGroup<S>[] {
    ...
    for (let label of this.labels) {
      ...
      let end_line = src.get_offset_line(label.span.end.saturating_sub(1).max(label.span.start)).map(([_, l, __]) => l)

      let label_info = new LabelInfo<any>(
        start_line.equal(end_line)
          ? LabelKind.Inline
          : LabelKind.Multiline,
        label,
      );
      ...
      for (let idx of range(line_range.start, line_range.end)) {
        ...
        for (let row of range(0, line_labels.length)) {
          ...
          for (let col of range(0, arrow_len)) {
              ...
              let is_hbar = ((((col > line_label.col) ? 1 : 0) ^ (line_label.multi ? 1 : 0))
              ...
              const getctail = (): [any, any] => {
                if (col === line_label.col && line_label.label.msg.is_some() && margin_label.map_or(true, m => line_label.label != m.label)) {
                  ...
                  if (!this.config.cross_gap && is_hbar) {
                    return [new Display(draw.xbar).fg(line_label.label.color), new Display(' ').fg(line_label.label.color)]
                  } else if (is_hbar) {
                    let d = new Display(draw.hbar).fg(line_label.label.color);
                    return [d, d]
                  } else {
                    return [
                      new Display((vbar_ll.multi && (row === 0) && this.config.compact)
                        ?  draw.uarrow
                        :  draw.vbar
