...
export class Source implements Cache<string> {
  ...
  chars(): string {
      return this.lines().map(l => l.chars()).flat() as any // TODO
  }
  ...
  get_line_range(span: Span): Range {
    ...
    let end = this.get_offset_line(span.end.saturating_sub(1).max(span.start)).map_or(this.lines().length, ([_, l, __]) => l + 1);
    // start..end
...
export class FileCache implements Cache<Path> {
  constructor(
    public files: Map<PathBuf, Source>,
  ) {}
  ...
  static is(other: any): other is FileCache { return other instanceof FileCache }
}
...
export class FnCache<Id, F extends Function> implements Cache<Id> {
  ...
  static new<Id, F extends Function>(get: F): FnCache<Id, F> {
    return new FnCache<Id, F>(
      new Map()/* HashMap::default() */,
      get,
    )
  }
  ...
  display(id: any): Option<Displayable> { return some(id) }

  static is(other: any): other is FnCache<any, any> { return other instanceof FnCache }
}
